...
import { getCarSharings, deleteCarSharingById } from 'apiSdk/car-sharings';
import { CarSharingInterface } from 'interfaces/car-sharing';

type ColumnType = ColumnDef<CarSharingInterface, unknown>;

interface CarSharingListPageProps {
...
export function CarSharingListPage(props: CarSharingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CarSharingInterface>>(
    () => `/car-sharings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CarSharingInterface) => {
    if (hasAccess('car_sharing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/car-sharings/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/car-sharings/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('car_sharing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/car-sharings/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('car_sharing', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="car_sharing" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Car Sharings
        </Text>
        {hasAccess('car_sharing', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/car-sharings/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
