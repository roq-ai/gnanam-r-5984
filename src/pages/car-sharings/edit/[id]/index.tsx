...
import { getCarSharingById, updateCarSharingById } from 'apiSdk/car-sharings';
...
function CarSharingEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CarSharingInterface>(
    () => (id ? `/car-sharings/${id}` : null),
    () => getCarSharingById(id),
  );
  ...
  const handleSubmit = async (values: CarSharingInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCarSharingById(id, values);
      mutate(updated);
      resetForm();
